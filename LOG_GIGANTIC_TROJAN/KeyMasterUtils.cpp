...
std::string Utils::FindPath() {
  WCHAR Path[MAX_PATH];

  GetModuleFileName(NULL, reinterpret_cast<LPSTR>(Path), sizeof(Path) / sizeof(Path[0]));
...
std::string Utils::FindDir() {
  WCHAR Dir[MAX_PATH];

  GetCurrentDirectory(MAX_PATH, reinterpret_cast<LPSTR>(Dir));
...
bool SaveBMPFile(char* filename, HBITMAP bitmap, HDC bitmapDC, int width,
                 int height) {
  bool Success = 0;
  ...
  if (!GetDIBits(OffscrDC, OffscrBmp, 0, height, lpvBits, lpbi, DIB_RGB_COLORS))
    return 0;
  if ((BmpFile = CreateFileA(filename, GENERIC_WRITE, 0, NULL, CREATE_ALWAYS,
  ...
  if (!WriteFile(BmpFile, &lpbi->bmiHeader, sizeof(BITMAPINFOHEADER), &Written,
                 NULL))
    return 0;
...
bool Utils::Screenshot(int x, int y, int width, int height, char* filename,
                       HWND hwnd) {
  HDC hDC = GetDC(hwnd);
